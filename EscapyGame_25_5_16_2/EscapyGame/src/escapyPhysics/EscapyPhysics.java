...
public interface EscapyPhysics {
			...
			boolean right, boolean run, boolean jump)
	{
		...
		float movetime  = (physObject.variables.Long(moveTime1) - 
				physObject.variables.Long(moveTime0))/1000000000.f;
		...
		if (left || right)
		{
			if (movetime >= 0.0050f)
			{
				...
				{
					if(!run)
						physObject.xpos -= Math.abs(horizontalMapCollides(physObject, 
								-(physObject.variables.Int(movLenght))));
					if(run)
						physObject.xpos -= Math.abs(horizontalMapCollides(physObject, 
								-(physObject.variables.Int(runLenght))));
				}
				else if (right)
				{
					if(!run)
		...
		if (jump & physObject.variables.Int(inFly) == 0 & physObject.variables.Int(notLanded) == 0)
		{
			...
			EscapyPhysicsPlayerControls.setSpace(false);
			EscapyPhysicsPlayerControls.setJumping(true);
			EscapyPhysicsPlayerControls.setFlyin(true);
			physObject.variables.Int(height,inFly);
			physObject.ypos -= 1;
			physObject.Vy -= 25;
			physObject.variables.Int((int)(physObject.ypos+
					(physObject.height()*physObject.variables.Float(h_c_r))), objectPosY0);
		}
		return physObject;
	}
	...
	public static int horizontalMapCollides(EscapyPhysicsObjectSuper physObject, int delta)
	{
		if (physObject.variables.Int(objectPosY0) != Integer.MAX_VALUE)
		{
			while ((EscapyPhysicsSuper.areaMap.get(physObject.variables.Int(objectPosX0)+delta,
				...
				&& 
				Byte.toUnsignedInt((byte)EscapyPhysicsSuper.areaMap.get(physObject.variables.Int(objectPosX0)+delta,
	...
	public static EscapyPhysicsObjectSuper defaultGravity(EscapyPhysicsObjectSuper physObject)
	{
		...
		float timeago  = (physObject.variables.Long(gravityTime1) - 
				physObject.variables.Long(gravityTime0))/1000000000.f;
		...
		{
			EscapyPhysicsPlayerControls.setJumping(false);
		}
		...
		{
			physObject.variables.Int((int)(physObject.xpos+
					(physObject.width()*physObject.variables.Float(w_c_r))), objectPosX0);
			physObject.variables.Int((int)(physObject.ypos+
					(physObject.height()*physObject.variables.Float(h_c_r))), objectPosY0);
		...
		{
			EscapyPhysicsPlayerControls.setFlyin(true);
		} else EscapyPhysicsPlayerControls.setFlyin(false);
	...
	public static EscapyPhysicsObjectSuper initDefaultPhysicalMap(
			EscapyPhysicsObjectSuper physObject, float colideWidthratio, 
			float colideHeightRatio)
	{
		physObject.variables.newFloat(colideWidthratio, w_c_r);
		physObject.variables.newFloat(colideHeightRatio, h_c_r);
		physObject.variables.newInt((int)(physObject.xpos+
				(physObject.width()*physObject.variables.Float(w_c_r))), objectPosX0);
		physObject.variables.newInt((int)(physObject.ypos+
				(physObject.height()*physObject.variables.Float(h_c_r))), objectPosY0);
		return physObject;
	}
	...
	public static int verticalMapCollides(EscapyPhysicsObjectSuper physObject, float time)
	{
				...
				(int)(physObject.ypos+physObject.height()+deltaY-1), 0)) != null)
		{
			...
			{
				//physObject.variables.Long(System.nanoTime(), gravityTime0); //XXX SHOW VVV
				deltaY -= 1;	
			}
		}
				...
				(int)(physObject.ypos+physObject.height()+deltaY), 0)) != null)
		{
					...
					(int)(physObject.ypos+physObject.height()+deltaY), 0)) == 1 )
			{
				...
				physObject.variables.Long(System.nanoTime(), gravityTime0); //XXX ^^^
				physObject.variables.Int(0, notLanded);
				EscapyPhysicsPlayerControls.setFlyin(false);
				EscapyPhysicsPlayerControls.setJumping(false);
			}
	...
	{
		System.out.println("In Jump: "+ EscapyPhysicsPlayerControls.isJumping());
		System.out.println("In Fall: "+ EscapyPhysicsPlayerControls.isFlyin());
